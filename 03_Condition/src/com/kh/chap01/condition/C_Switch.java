...
public class C_Switch {
	...
	public void method0() {
		...
		case "B3" : System.out.println("지하 3층입니다. 문이 열립니다."); break;
		}
	}
	 ...
	 * 
	 * switch(정수 || 실수 || 문자 || 문자열) {
	 * case 값1 : 실행할코드a; // 동등비교대상 == 값1 true일 경우 실행할 코드 a를 수행
	 * 
	 * case 값2 : 실행할코드b; // 동등비교대상 == 값2 true일 경우 실행할 코드 b를 수행
	 * 
	 * case 값n : 실행할코드n; // 동등비교대상 == 값n true일 경우 실행할코드 n을 수행
	 * 
	 * default : 실행할코드; // == else
	 * }
	 */
	...
	public void method1() {
		...
		Scanner sc = new Scanner(System.in);
		System.out.println("메뉴를 선택해주세요(부찌, 떡갈비, 제육, 칼국수, 수제비) ");
		...
		*/
		...
		default : System.out.println("그런 메뉴는 없습니다");
		return;	// 현재 실행되고 있는 메소드 영역을 아예 빠져 나감
				// 메소드를 호출한 곳으로 돌아감
		} // break를 만나면 빠져나오는 부분
	...
	public void login() {
		...
		// 1. 회원가입 시 사용자가 원하는 아이디와 비밀번호 값을 입력하여 회원가입을 진행
		// 2. 가입 시 입력했던 아이디와 비밀번호를 둘 다 정확히 입력 한다면 로그인 성공 화면 출력
		...
		// 만약에 사용자가 로그인 시 입력한 아이디값과 회원가압 시 가입한 아이디 값이 동일하고
		...
		// = > 기본 자료형의 경유 동등비교 연산자를 수행하는데 있어 문제가 없음(==, !=)

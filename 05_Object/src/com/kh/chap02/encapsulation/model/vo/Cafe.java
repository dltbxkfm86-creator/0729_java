...
public class Cafe {
	 ...
	 * 2. set필드명으로 식별자를 작성하며 낙타봉표기법(camelCase)을 꼭 지키도록 하자.
	 * 
	...
	public void setSignatureMaenuPrice(int signatureMenuPrice) {
		...
		if(signatureMenuPrice <= 1000) {
			System.out.println("정상적이지 않은 값을 입력하셨습니다. 1000 이상의 정수를 입력해주세요");
		} else {
		...
		/*
		 * 관례 위반 두 개 이상의 필드값을 변 겨하는 메소드는 set이 붙으면 XX
		 * 
		 ...
		 * 2. get필드명으로 짓되, 낙타표기법(camelCase)를 사용한다
		...
		public String getAddress() {
			...
			// 메소드의 반환형과 반환하고자 하는 값의 자료형이 일치하는가를 꼭 잘 확인해야 함
		}
		...
		// getter() / setter() 다 만듦 => 캡슐화가 끝남(문법적인 관례)
		/*
		 * 1. 값을 숨긴다 => 필드의 접근제한자를 public대신 private으로 선언한다
		 * 2. 메소드를 통해 필드에 접근할 수 있도록 만들어줌 => getter() / setter() 구현
		 * 
		...
		public String info() {
			...
			// return 할 때 반환할 수 있는 값 단 한개
			String info = "[name 필드값 : " + name + ", address 필드값 : " 
